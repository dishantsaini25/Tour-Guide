...
const IV = "#F2F0EC", SF = "#E8F0EC", WH = "#FFFFFF", BD = "#C8D8D0";

const LABEL = { fontFamily:IN, fontSize:"0.6rem", letterSpacing:"0.28em", textTransform:"uppercase", fontWeight:700, color:CP };
const RULE  = { width:"40px", height:"1.5px", background:`linear-gradient(to right,${CP},#C89A5A)`, margin:"10px 0 22px" };
...
export default async function ExperienceDetailPage({ params }) {
  ...
  return (
    <>
      <style>{`
        .sb-book{display:flex;align-items:center;justify-content:center;gap:7px;background:${FR};color:#F0EDE8;padding:14px;font-family:${IN};font-size:.82rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;text-decoration:none;transition:background .2s;}
        .sb-book:hover{background:#163529!important;}
        .sb-wa{display:flex;align-items:center;justify-content:center;gap:7px;border:1px solid ${BD};color:${MU};padding:12px;font-family:${IN};font-size:.82rem;font-weight:500;text-decoration:none;transition:all .2s;}
        .sb-wa:hover{border-color:#25D366!important;color:#25D366!important;}
        .back-link{display:inline-flex;align-items:center;gap:6px;color:rgba(240,237,232,0.7);font-family:${IN};font-size:.8rem;font-weight:500;text-decoration:none;transition:color .2s;}
        .back-link:hover{color:#F0EDE8!important;}
        .mob-book-btn{flex:1;display:flex;align-items:center;justify-content:center;gap:6px;background:${FR};color:#F0EDE8;height:100%;font-family:${IN};font-size:.82rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;text-decoration:none;transition:background .2s;padding:0 16px;}
        .mob-book-btn:hover{background:#163529!important;}
        .mob-wa-btn{display:flex;align-items:center;justify-content:center;gap:6px;border:1.5px solid ${BD};color:${CH};height:44px;padding:0 16px;font-family:${IN};font-size:.82rem;font-weight:600;text-decoration:none;transition:all .2s;white-space:nowrap;}
      ...
      <section style={{ position:"relative", minHeight:"65vh", display:"flex", alignItems:"flex-end", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", inset:0, background:"linear-gradient(to top,rgba(10,25,16,0.92) 0%,rgba(10,25,16,0.25) 50%,rgba(10,25,16,0.15) 100%)" }} />
        <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse at 15% 80%,rgba(30,77,58,0.25) 0%,transparent 55%)" }} />
        ...
        <div style={{ position:"relative", zIndex:10, maxWidth:"1320px", margin:"0 auto", padding:"0 20px 48px", width:"100%", paddingTop:"140px" }} className="inner-pad">
          <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"14px" }}>
            <div style={{ width:"28px", height:"1px", background:CP }} />
            <span style={{ ...LABEL }}>{exp.theme}</span>
          </div>
          <h1 style={{ fontFamily:PF, fontWeight:700, fontSize:"clamp(2.4rem,6.5vw,5.5rem)", color:"#F0EDE8", lineHeight:1.05, marginBottom:"12px" }}>{exp.title}</h1>
          <p style={{ fontFamily:PF, fontSize:"clamp(1rem,2vw,1.5rem)", fontStyle:"italic", color:"rgba(240,237,232,0.68)", marginBottom:"10px" }}>{exp.subtitle}</p>
          <p style={{ fontFamily:IN, fontSize:"0.9rem", color:"rgba(240,237,232,0.42)", fontStyle:"italic", fontWeight:300 }}>{exp.question}</p>
        </div>
      </section>

      {/* ── Quick Info Strip ── */}
      <div style={{ background:SF, borderBottom:`1px solid ${BD}` }}>
        <div className="quick-info-strip" style={{ maxWidth:"1320px", margin:"0 auto", padding:"18px 20px", display:"flex", flexWrap:"wrap", gap:"24px" }}>
          {[
            ...
            <div key={label} style={{ display:"flex", alignItems:"center", gap:"10px", flexShrink:0 }}>
              <Icon size={14} style={{ color:CP }} />
              <div>
                <p style={{ fontFamily:IN, fontSize:"0.56rem", textTransform:"uppercase", letterSpacing:"0.18em", color:"#7A8A84", marginBottom:"1px" }}>{label}</p>
      ...
      {/* ── Main Content ── */}
      <div className="detail-page-content" style={{ background:IV, padding:"48px 0" }}>
        <div style={{ maxWidth:"1320px", margin:"0 auto", padding:"0 20px" }} className="inner-pad">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
            ...
            <div className="lg:col-span-2" style={{ display:"flex", flexDirection:"column", gap:"48px" }}>
                ...
                <ul style={{ display:"flex", flexDirection:"column", gap:"14px" }}>
                  {exp.whatYouExperience.map((item, i) => (
                    <li key={i} style={{ display:"flex", alignItems:"flex-start", gap:"12px" }}>
                      <CheckCircle2 size={16} style={{ color:CP, marginTop:"3px", flexShrink:0 }} />
              ...
              {/* Highlights */}
              <div style={{ background:WH, border:`1px solid ${BD}`, borderLeft:`4px solid ${FR}`, padding:"28px" }}>
                <p style={LABEL}>Highlights</p>
                <div style={{ width:"36px", height:"1.5px", background:CP, margin:"10px 0 20px" }} />
              ...
              <div>
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:"10px" }}>
                  {exp.idealFor.map(item => (
                    <span key={item} style={{ background:SF, border:`1px solid ${BD}`, color:CH, fontFamily:IN, fontSize:"0.82rem", fontWeight:500, padding:"8px 16px" }}>{item}</span>
                  ))}
                </div>
              </div>
                  ...
                  <ul style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                    {exp.inclusions.map(item => (
                      <li key={item} style={{ display:"flex", alignItems:"flex-start", gap:"10px" }}>
                        <CheckCircle2 size={14} style={{ color:CP, marginTop:"3px", flexShrink:0 }} />
                ...
                <div>
                  ...
                  <ul style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                    {exp.exclusions.map(item => (
                      <li key={item} style={{ display:"flex", alignItems:"flex-start", gap:"10px" }}>
                        <XCircle size={14} style={{ color:"#7FA89A", marginTop:"3px", flexShrink:0 }} />
                        <span style={{ fontFamily:IN, color:MU, fontSize:"0.875rem", fontWeight:300 }}>{item}</span>
                      </li>
                    ))}
                  </ul>
              ...
              <div>
                ...
                <div style={{ border:`1px solid ${BD}`, overflow:"hidden" }}>
                  {[
                    ...
                    <div key={k} style={{ display:"flex", borderBottom: i < 6 ? `1px solid ${BD}` : "none" }}>
                      <div style={{ width:"38%", padding:"13px 18px", background:SF, fontFamily:IN, fontSize:"0.7rem", textTransform:"uppercase", letterSpacing:"0.12em", color:MU, fontWeight:600, display:"flex", alignItems:"center" }}>{k}</div>
              ...
              {/* Curator note */}
              <div style={{ borderLeft:`3px solid ${CP}`, padding:"22px 22px 22px 28px", background:"rgba(176,125,62,0.04)" }}>
                ...
                <div>
                  ...
                  <div className="gallery-carousel">
                    {exp.gallery.map((img, i) => (
                      <div key={i} style={{ position:"relative", aspectRatio:"4/3", overflow:"hidden", borderRadius:"2px", border:`1px solid ${BD}` }}>
            ...
            <div className="lg:col-span-1 detail-sidebar">
              <div style={{ position:"sticky", top:"88px", display:"flex", flexDirection:"column", gap:"14px" }}>
                <div style={{ background:WH, border:`1px solid ${BD}` }}>
                  <div style={{ background:FR, padding:"22px 24px" }}>
                    <h3 style={{ fontFamily:PF, fontSize:"1.5rem", fontWeight:700, color:"#F0EDE8", marginBottom:"4px" }}>Book This Experience</h3>
                    <p style={{ fontFamily:IN, color:"rgba(240,237,232,0.5)", fontSize:"0.78rem", fontWeight:300 }}>We respond personally within 24 hours</p>
                  </div>
                  <div style={{ padding:"22px 24px" }}>
                    {[["Duration",exp.duration],["Start Time",exp.startTime],["Group Size",exp.groupSize]].map(([k, v]) => (
                      <div key={k} style={{ display:"flex", justifyContent:"space-between", paddingBottom:"10px", marginBottom:"10px", borderBottom:`1px solid ${BD}` }}>
                        <span style={{ fontFamily:IN, color:"#7A8A84", fontSize:"0.78rem", fontWeight:300 }}>{k}</span>
                ...
                <div style={{ padding:"16px 18px", background:SF, border:`1px solid ${BD}` }}>
                  <p style={{ fontFamily:IN, fontSize:"0.8rem", color:MU, lineHeight:1.7, fontWeight:300 }}>
                    ...
                    <Link href="/contact" style={{ color:CP, textDecoration:"none" }}>Get in touch.</Link>
                  </p>
                </div>

                <div style={{ padding:"16px 18px 16px 20px", borderLeft:`2px solid ${CP}`, background:WH }}>
      ...
      {/* Related */}
      {related.length > 0 && (
        <div style={{ background:SF, padding:"80px 0" }}>
          <div style={{ maxWidth:"1320px", margin:"0 auto", padding:"0 20px" }} className="inner-pad">
            <div style={{ textAlign:"center", marginBottom:"52px" }}>
              <p style={LABEL}>Continue Exploring</p>
              <div style={{ width:"40px", height:"1.5px", background:`linear-gradient(to right,${CP},#C89A5A)`, margin:"10px auto 18px" }} />
